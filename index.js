...
app.post("/stories",async (request,response)=>{
    const data=request.body;
    const result=await client.db("test").collection("stories").insertMany(data);
    response.send(result);
});
app.delete("/stories",async (request,response)=>{
    const data=request.body;
    const result=await client.db("test").collection("stories").deleteMany(data);
    response.send(result);
});
app.get("/rhymes",async (request,response)=>{
    // response.send(rhymes);
    const rhymes=await client
    .db("test")
    .collection("rhymes")
    .find({})
    .toArray();
...
app.post("/rhymes",async (request,response)=>{
    const data=request.body;
    const result=await client.db("test").collection("rhymes").insertMany(data);
    response.send(result);
});
...
app.post("/home",async (request,response)=>{
    const data=request.body;
    const result=await client.db("test").collection("child").insertMany(data);
    response.send(result);
});
